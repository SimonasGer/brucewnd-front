...
export const Comic = () => {
    ...
    const handlePublishToggle = async () => {
        ...
        try {
            ...
            await axios.patch( endpoint, { publish: !comic.isPublished },
                { headers: { ...authHeaders() } }
            );
        } catch (err) {
            // revert on fail
        ...
        try {
            ...
            try {
                const res = await axios.get(
                    `${apiUrl}/comics/${encodeURIComponent(comic.name)}`
                );
                setComic(res.data);
            } catch {}
        } finally {
    ...
    const handleDeleteComic = async () => {
        if (!isAdmin) return;
        if (!window.confirm("Delete the entire comic? This cannot be undone."))
            return;
        ...
        <main>
            ...
            {!loading && !error && (
                <section className="comic-details">
                    ...
                    <p>
                        Published:{" "}
                        <strong>{comic.isPublished ? "Yes" : "No"}</strong>
                    </p>
                    <p>
                        Tags:{" "}
